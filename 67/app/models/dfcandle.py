    ...
    def back_test_ema(self, period_1: int, period_2: int):
        ...
        for i in range(1, len(self.candles)):
            ...
            if ema_value_1[i-1] < ema_value_2[i-1] and ema_value_1[i] >= ema_value_2[i]:
                signal_events.buy(product_code=self.product_code, time=self.candles[i].time, price=self.candles[i].close, amount=1.0, save=False)

            if ema_value_1[i-1] > ema_value_2[i-1] and ema_value_1[i] <= ema_value_2[i]:
                signal_events.sell(product_code=self.product_code, time=self.candles[i].time, price=self.candles[i].close, amount=1.0, save=False)
