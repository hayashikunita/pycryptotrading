...
class SignalEvents(object):
    ...
    def buy(self, product_code, time, price, amount, save):

        # pyfxtradingの場合(time関連の記述が関連すると、エラーが起こる為)
        # if not self.can_buy(time):
        #     return False
        # pyfxtradingの場合

        signal_event = SignalEvent(
            time=time, product_code=product_code, side=constants.BUY, price=price, amount=amount)
        if save:
            signal_event.save()
    ...
    def sell(self, product_code, time, price, amount, save):
        ...
        signal_event = SignalEvent(
            time=time, product_code=product_code, side=constants.SELL, price=price, amount=amount)
        if save:
            signal_event.save()
    ...
    def profit(self):
        ...
        for i in range(len(self.signals)):
            ...
            if signal_event.side == constants.BUY:
                total -= signal_event.price * signal_event.amount
                is_holding = True
